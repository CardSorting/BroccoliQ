...
export class BufferedDbPool {
	...
	private activeLocks = new Map<
		string,
		{ expiresAt: number; interval: NodeJS.Timeout }
	>();
	...
	private startFlushLoop() {
		...
		logger.info("BufferedDbPool initialized with sharding support.");
	}
	...
	private async ensureDb(shardId: string = "main"): Promise<Kysely<Schema>> {
	...
	/**
	 * Distributed Mutex: Level 8 Sovereign Lock.
	...
	public async acquireLock(
		...
		try {
			// 1. Try to clean up expired locks first
			...
			// We need to wait for the flush or run it immediately for locks
			await this.flush();

			// 3. Verify ownership (Double-check after flush)
			const claim = await this.selectOne(
				"claims",
				[
			...
			if (claim) {
				...
				const interval = setInterval(
					() => this.heartbeatLock(resource, author, shardId, ttlMs),
					ttlMs / 2,
				);
	...
	private async heartbeatLock(
		...
		try {
			...
			logger.error(`[DbPool] Heartbeat failed for ${resource}`, e);
		}
	}
	...
	public async pushBatch(
		...
		if (agentId) {
			...
			for (const op of ops) {
				shadow.ops.push({ ...op, agentId, shardId: op.shardId || "main" });
			}
		...
		if (currentBufferLength > 100000) {
			logger.warn(
				`[DbPool] CRITICAL backpressure: activeBuffer length is ${currentBufferLength}`,
			);
		}
	...
	public async runTransaction<T>(
		callback: (agentId: string) => Promise<T>,
	): Promise<T> {
		...
		try {
			...
			try {
				...
				if (hasData) {
					...
					this.activeBuffer =
						dirtyBuffer === this.bufferA ? this.bufferB : this.bufferA;
					...
					opsToFlush = Array.from(dirtyBuffer.values())
						...
						.sort((a, b) => {
							...
							if (a.table !== b.table)
								return (a.table as string).localeCompare(b.table as string);
			...
			await db.transaction().execute(async (trx) => {
				...
				for (const group of processedGroups) {
					...
					const shardId = first.shardId || "main";

					// High-Performance Path: Chunked Raw SQL (Level 3 Quantum Boost)
					const raw = await this.getRaw(shardId);
					if (group.length >= 100 && first.type === "insert") {
						totalFlushed += await this.executeChunkedRawInsert(
							table,
							group,
							raw,
						);
	...
	private async executeBulkUpdate(
		...
		if (
			canBatchIntoSingleStatement &&
			first.where &&
			!Array.isArray(first.where)
		) {
			const ids: unknown[] = [];
			for (const op of group) {
	...
	public async selectWhere<T extends keyof Schema>(
		...
		try {
			...
			if (!isWarmed) {
				...
				if (options?.orderBy) {
					query = query.orderBy(
						options.orderBy.column as never,
						options.orderBy.direction as never,
					);
				}
				if (options?.limit) {
			...
			applyOps(
				this.inFlightOps.get(table) || [],
				this.inFlightIndex.get(table),
				finalResults,
			);
			applyOps(
				this.activeBuffer.get(table) || [],
				this.activeIndex.get(table),
				finalResults,
			);
			if (agentId) {
				const shadow = this.agentShadows.get(agentId);
			...
			logger.error("selectWhere query failed", e, { table, shardId });
			return [];
		} finally {
			release();
		}
				...
				if (existingIdx !== undefined) {
					...
					if (targetOp?.values && op.values) {
						for (const [key, val] of Object.entries(op.values)) {
							const existingVal: unknown = targetOp.values[key];
							const isInc = (v: unknown): v is Increment =>
								!!v &&
								typeof v === "object" &&
								"_type" in v &&
								(v as Record<string, unknown>)._type === "increment";
	...
	private async executeChunkedRawInsert(
		...
		for (let i = 0; i < group.length; i += CHUNK_SIZE) {
			...
			const stmt = this.getStatement(sqlStr, raw);
	...
	private async executeSingleOp(trx: Transaction<Schema>, op: WriteOp) {
		...
		if (op.type === "insert" && op.values) {
			await trx
				.insertInto(op.table)
				.values(op.values as never)
				.execute();
		} else if (op.type === "upsert" && op.values) {
			let query = trx.insertInto(op.table).values(op.values as never);
			if (op.conflictTarget) {
				const targets = Array.isArray(op.conflictTarget)
					? op.conflictTarget
					: [op.conflictTarget];
				query = query.onConflict((oc) =>
					oc.columns(targets as never).doUpdateSet(op.values as never),
				);
			} else {
				query = query.onConflict((oc) =>
	...
	public async warmupTable<T extends keyof Schema>(
		...
		const rows = await db
			...
			.where(statusCol as never, "=", statusValue as never)
			.selectAll()
			.execute();

		if (rows.length === 0) return 0;

		let tableIndex = this.activeIndex.get(table as never);
		if (!tableIndex) {
			tableIndex = new Map();
			this.activeIndex.set(table as never, tableIndex);
		}
